...
class LoginSerializer(serializers.Serializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if config.AUTHENTICATION_USER_SERIALIZER:
            user_serializer_class: Type[serializers.Serializer] = import_string(
                config.AUTHENTICATION_USER_SERIALIZER
            )
    ...
    def validate(self, data):
        ...
        credentials = {self.username_field: username, "password": data.get("password")}
        protection = None
        if (
            apps.is_installed("rest_framework_security.allowed_ips")
            and get_user_model().objects.filter(username=username).exists()
        ):
            from rest_framework_security.allowed_ips.protection import (
                AllowedIpsProtection,
            )
        ...
        if user is None and get_user_model().objects.filter(username=username).exists():
            logger.info(
                f"Authentication error for user {username} from ip {get_client_ip(request)}"
            )
        elif user is None:
            logger.info(
    ...
    def create(self, validated_data):
        ...
        user_sessions: UserSessionQuerySet = UserSession.objects.filter(
        ...
        user_sessions.apply_max_active_sessions(
            config.AUTHENTICATION_MAX_ACTIVE_SESSIONS
        )
        next_steps = list(get_next_required_steps(request))
        logger.info(
            f'User {validated_data["user"]} authenticated from ip {ip_address}. '
            f'Next steps: {",".join(next_steps) or "nothing"}'
        )
        return {
            "user": validated_data["user"],
            "session_expires": datetime.datetime.now()
            + datetime.timedelta(seconds=session_age),
            "max_session_renewal": max_session_renewal,
